/*
 * NTRG GMP WINCE -- memory.c
 * Patroklos G. Argyroudis <argp@cs.tcd.ie>
 *
 * Modified version of memory.c for compilation of GMP for WinCE.
 *
 * $Id: memory.c,v 1.2 2004/01/11 10:32:38 argp Exp $
 */
...
#endif
{
  ...
  if (ret == 0)
    {
      fprintf(stderr, "gmp: (malloc) cannot allocate in gmp\n");
      abort();
    }
...
#endif
{
  ...
  if (ret == 0)
    {
      fprintf(stderr, "gmp: (realloc) cannot allocate in gmp\n");
      abort();
    }
